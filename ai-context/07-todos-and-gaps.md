...
|-----|-----|-------|--------|
| ~~System-audio loopback not implemented~~ → **IMPLEMENTED** | 🟢 | [`apps/desktop/src/main/loopback.ts`](../apps/desktop/src/main/loopback.ts), [`renderer/audio/capture-streams.ts`](../apps/desktop/src/renderer/audio/capture-streams.ts) | Loopback now works via Electron `getDisplayMedia({audio:true})` → main `setDisplayMediaRequestHandler` returning `audio:'loopback'` (ScreenCaptureKit macOS 13+ / WASAPI Windows, **no native addon**). The renderer offers mic / system / both (mic+system mixed into one 16 kHz stream) and **gates system capture behind a one-time in-app consent disclosure** ([`hooks/use-consent.ts`](../apps/desktop/src/renderer/hooks/use-consent.ts)). Residual: macOS needs Screen Recording permission granted (runtime prompt); no per-speaker diarization yet (both sides are mixed); the descoped full legal/consent framework still does not exist — the in-app gate is a disclosure, not legal cover. `packages/core` `NotImplementedLoopbackCapture` is now just a placeholder for a possible future native per-process tap. |
| Local-whisper in Electron — wired; packaged build unverified | 🟢 | [`apps/desktop/electron.vite.config.ts`](../apps/desktop/electron.vite.config.ts), [`electron-builder.yml`](../apps/desktop/electron-builder.yml) | **Done for dev:** `smart-whisper` is a desktop optionalDependency, force-externalized in electron-vite, rebuilt for Electron via `pnpm --filter @cue/desktop rebuild:native`, and **verified loading in the Electron main process** (smoke test: `Whisper=true`, no ABI mismatch). So local mode does free STT with no backend. Remaining: the `.dmg`/`.exe` path relies on electron-builder's `npmRebuild` + `asarUnpack` — configured but **not yet verified by a real packaged-app run** (couldn't launch a signed bundle headlessly). |
| Local-whisper latency vs Deepgram | 🟢 | same | Whisper transcribes in chunks (~1–2 s) vs Deepgram's ~0.3 s interim. Tunable via `intervalMs`/`silenceMs`/model size; acceptable for cues, noticeably less snappy than cloud. |
