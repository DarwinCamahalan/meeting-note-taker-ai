...
| Local-whisper in Electron needs an ABI rebuild | 🟡 | [`packages/core/src/stt/local-whisper-client.ts`](../packages/core/src/stt/local-whisper-client.ts) | Free local STT (`smart-whisper`, a native addon) is **verified** in plain Node (core test + the `ai-orchestrator` gateway path). In the desktop **local** mode `@cue/core` runs inside Electron's main process, whose Node ABI differs — the addon must be rebuilt for Electron (`electron-rebuild`) and asar-unpacked when packaging. Until wired, local mode fails loud (overlay error) and the free path is `CUE_BACKEND=gateway`. `TODO`: add electron-rebuild + `asarUnpack` for the `.dmg`/`.exe`. |
